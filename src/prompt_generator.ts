...
1. 주어진 텍스트에 대한 효과적인 기억을 돕는 3~7개 내외의 '기억 카드'를 만드세요. 텍스트의 분량과 복잡성에 따라 2~15개 내외로 유연하게 조절 가능하며, 핵심 개념을 빠짐없이 다루는 것을 우선시합니다. 오직 주어진 텍스트만을 활용해서 기억 카드를 생성하세요.

2. 중요한 형식 요구사항:
   - 질문은 한국어로 작성하고 답변도 한국어로 작성
   - 각 카드는 반드시 하나의 개념에 집중하고 정확하고 명확해야 함, 실제 회상이 필요해야 함 (너무 명백하지 않아야 함)`;
...
5. 각 문장에는 1~2개의 핵심 단어만 {{}}로 감싸세요. 너무 많은 단어를 감싸면 학습 효과가 떨어집니다.

6. Cloze 카드는 반드시 2개 이상 생성해야 합니다 (단, 주어진 텍스트가 매우 짧거나 단일 개념일 경우 1개도 허용)
...
5. 모든 카드는 반드시 질문/답변 형식이어야 합니다. Cloze 형식의 카드는 생성하지 마세요.
...
3. Cloze 기억 카드는 다름 형식을 따릅니다: 
   - 앞면: 핵심 단어를 {{}}로 감싼 문장 (중요: 질문 형식이 아닙니다. 마침표로 끝나야 함.)
   - 뒷면: 앞면과 **동일한 문장**에서 핵심 단어를 {{}} 대신 **볼드** 처리한 문장
   - Cloze 기억 카드는 반드시 2개 이상 생성해야 합니다 (단, 주어진 텍스트가 매우 짧거나 단일 개념일 경우 1개도 허용)
   - Cloze 카드 예시: {{간헐적 복습}}은 {{장기 기억}}에 유리하다. / **간헐적 복습**은 **장기 기억**에 유리하다.
