...
import { transcribeAudio } from './audio_processor';
...
import { processSnippet } from './snippet_processor';
...
export const processTextPipeline = functions.http('processTextPipeline', async (req, res) => {
    ...
    try {
        ...
        if (processType === 'markdown') {
            ...
            // 스니펫 처리 함수 호출
            const result = await processSnippet(req.body);
...
// === 오디오 트랜스크립션 엔드포인트 ===
export const processAudioTranscription = functions.http('processAudioTranscription', async (req, res) => {
    // CORS 설정
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
...
// === 스니펫 생성 엔드포인트 ===
export const processSnippetCreation = functions.http('processSnippetCreation', async (req, res) => {
    // CORS 설정
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
