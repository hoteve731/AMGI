...
// 스니펫 생성 프롬프트 템플릿
const getSnippetPrompt = (headerText: string, snippetType: string, contentMarkdown: string = '', language: string = 'Korean', customQuery?: string) => {
    // 언어 설정에 따른 프롬프트 조정
    let langPrefix = '';
    if (language === 'English') {
...
// 태그 추출 프롬프트
const getTagsPrompt = (headerText: string, snippetContent: string, language: string = 'Korean') => {
    // 언어 설정에 따른 프롬프트 조정
    let langPrefix = '';
    if (language === 'English') {
...
export async function processSnippet(data: any) {
    try {
