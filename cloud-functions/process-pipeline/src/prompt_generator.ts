...
2. 다음 마크다운 요소를 적극적으로 활용하세요:
   ...
   - 구분선(---): 주요 섹션 사이에 구분선을 넣어 가독성 향상

3. 이모티콘을 적절히 활용하여 시각적 매력을 높이세요:
   - 섹션 헤더 앞에 관련 이모티콘 추가 (예: 📚 학습 자료, 🔑 핵심 개념, ⚠️ 주의사항)
   - 목록 항목에 관련 이모티콘 사용 (예: ✅ 완료된 항목, 🔄 진행 중인 항목)
   - 과도하게 사용하지 말고 내용의 이해를 돕는 방향으로 적절히 사용
...
5. 고급 포맷팅 기법:
   - 마인드맵 형태의 계층 구조 표현 (중첩 리스트 활용)
   - 주요 개념에 대한 정의와 예시를 표로 정리
   - 프로세스나 단계는 넘버링하고 각 단계별 부가 설명은 중첩 불릿으로 처리
   - 핵심 요약은 문서 상단에 > 인용구로 배치

${additionalMemory
         ? `6. 사용자가 특별히 기억하고 싶어하는 내용: "${additionalMemory}"
   이 내용과 관련된 부분은 다음과 같이 특별 처리하세요:
   - ⭐ 이모티콘으로 시작하는 별도 섹션으로 분리
   - **볼드 처리**와 함께 박스(> 인용구 형태) 안에 강조
