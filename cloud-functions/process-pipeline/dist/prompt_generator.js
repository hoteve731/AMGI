...
3. 모든 카드는 아래 설명된 매우 엄격한 형식을 100% 준수해야 합니다. 형식 오류는 절대 허용되지 않습니다:
   ...
   3-1. Cloze 카드 형식 규칙 (아래 형식을 정확히 따르세요):
     - 앞면:
       - 문장의 핵심 개념/용어/정보를 나타내는 **하나 또는 두 개의 단어/짧은 구(phrase)**를 {{}}로 감싼 완전한 서술문이어야 합니다.
       - 절대 질문 형태(?)가 되어서는 안 됩니다. 반드시 마침표(.)로 끝나야 합니다.
       - {{}} 안의 내용은 내용을 떠올려야만 채울 수 있도록 신중하게 선택해야 합니다 ('Effortful' 원칙).
     - 뒷면:
       - 앞면 내용에서 {{}} 표시만 ** **(별표 두 개) 표시로 정확히 교체한 것 외에는, 100% 동일한 문장이어야 합니다. 절대 다른 내용이나 답변이 와서는 안 됩니다.
   ...
   3-2. 일반 카드 형식 규칙 (아래 형식을 정확히 따르세요):
     - 앞면:
       - 명확하고 구체적인 질문이어야 합니다.
       - 반드시 물음표(?)로 끝나야 합니다.
       - 질문 내의 **핵심 키워드(1~3개)**는 반드시 ** **(별 두 개)로 감싸서 강조해야 합니다. {{}}를 사용해서는 절대 안 됩니다.
     - 뒷면:
       - 질문에 대한 간결하고 정확한 답변이어야 합니다.
       - 답변 내의 **핵심 키워드(1~3개)**도 반드시 ** **(별 두 개)로 감싸서 강조해야 합니다.
     - 올바른 일반 예시: "카드 N: **간헐적 복습**이 **장기 기억**에 효과적인 이유는 무엇인가요? / **간헐적 복습**은 망각 곡선을 고려하여 **장기 기억**을 강화하기 때문입니다."
     - 잘못된 일반 예시 (절대 생성 금지):
       - 카드 N: {{핵심}}은 무엇인가요? / 답은 **핵심**입니다. (앞면에 {{}} 사용 불가)
       - 카드 N: 핵심 용어는 무엇인가요? / 핵심 용어입니다. (키워드 강조 ** ** 누락)
